...
df = pd.read_csv('C:/Users/nandi/OneDrive/Desktop/App-Ratings/data/mac_US_cleaned.csv', index=False)
...
paid_filter = st.sidebar.selectbox("Price Type", ["All", "Free", "Paid"])
...
with tab1:
    ...
    ).properties(title='Rating Distribution by Advisory Rating', width=700)
...
with tab2:
    ...
    for _, row in top_apps.iterrows():
        ...
        <div style='border:1px solid #ddd; padding:10px; margin:10px; border-radius:8px'>
            ...
            <p><strong>Price:</strong> {row['price_display']}</p>
...
st.markdown("---")
st.markdown("Made by Nandita Ghildyal | Powered by Streamlit & Hugging Face")